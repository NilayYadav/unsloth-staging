"""Scene: the web_search card after the model reads a dual-stack page on a host whose
IPv6 is configured but dead.

The change under review is in the BACKEND (`_validate_and_resolve_host` /
`_fetch_url_raw`), so nothing may be seeded: the model emits a `web_search` call
carrying the URL, Studio resolves the host, pins an address against DNS rebinding,
fetches the real page, streams the result to the SPA, and the SPA draws the card.

The network condition is the only thing arranged, and it is arranged for the
launched Studio process alone: `--studio-env PYTHONPATH=<dir>` where that dir holds
a `sitecustomize.py` making every connect() to a GLOBAL IPv6 address raise
ENETUNREACH. That is what the kernel returns on a configured-but-dead IPv6 default
route (a VPN with broken IPv6). Resolution is untouched, so the AAAA still sorts
first, exactly the ordering that makes the bug fire.

Two calls, in this order, because the pair is the control:

  1. the dual-stack page,   https://en.wikipedia.org/wiki/IPv6   (AAAA first, dead)
  2. an IPv4-only control,  https://httpbin.org/html             (no AAAA at all)

The control must load on BOTH sides: a side where it fails has a network or harness
problem, not the bug. The marker line the sitecustomize writes to the Studio log is
read back into the facts, so both sides prove the condition was live in the backend.
"""
...
MARKER = "[v6dead] global IPv6 connect() disabled"

DUAL_STACK_URL = "https://en.wikipedia.org/wiki/IPv6"
CONTROL_URL = "https://httpbin.org/html"

FAILURE_MARK = "Failed to fetch URL"
# The first words of _answer_for(): present only once both results are back.
ANSWER_MARK = "Reading the dual-stack page returned"
PAGE_MARK = "From Wikipedia"
CONTROL_MARK = "Moby-Dick"
...
def _answer_for(texts: list[str]) -> str:
    ...
    """
    dual = _summarise(texts[0] if texts else "")
    control = _summarise(texts[1] if len(texts) > 1 else "")
    return (
        f"Reading the dual-stack page returned {dual}. "
        f"Reading the IPv4-only control page returned {control}."
    )
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _emit_call(self, index: int, url: str) -> None:
            ...
            self.wfile.write(_sse(_delta({"tool_calls": [{
                "index": 0, "id": f"call_pr10319_{index}", "type": "function",
        ...
        def do_POST(self):  # noqa: N802
            ...
            if len(texts) == 0:
                self._emit_call(0, DUAL_STACK_URL)
            elif len(texts) == 1:
                self._emit_call(1, CONTROL_URL)
            else:
...
async def drive(
    ...
    *,
    prompt: str = (
        "Read https://en.wikipedia.org/wiki/IPv6, then read https://httpbin.org/html, "
        "and tell me what each one returned."
    ),
    **_: object,
) -> tuple[list[Path], dict]:
    """Photograph the two web_search cards: the dead-IPv6-first page, then the IPv4-only control."""
    ...
    try:
        ...
        async with open_chat(
            ...
            shot = out_dir / f"{label.lower()}_web_fetch_address_fallback.png"
            ...
            dual_text = texts[0] if texts else ""
            control_text = texts[1] if len(texts) > 1 else ""
            # The driver writes the Studio log next to this side's out_dir; the
            # sitecustomize marker in it is what proves the condition reached the
            # backend process on THIS side, not just the shell that launched it.
            log_path = out_dir.parent / f"{label.lower()}_studio.log"
            log_text = log_path.read_text(errors="replace") if log_path.exists() else ""
            facts = {
                "backend_ipv6_dead_marker": MARKER in log_text,
                ...
                "dual_stack_url_failed": dual_text.strip().startswith(FAILURE_MARK),
                "dual_stack_result_len": len(dual_text),
                "dual_stack_result_head": " ".join(dual_text.split())[:220],
                "dual_stack_has_page": PAGE_MARK in dual_text,
                "control_url_failed": control_text.strip().startswith(FAILURE_MARK),
                "control_result_len": len(control_text),
                "control_has_page": CONTROL_MARK in control_text,
