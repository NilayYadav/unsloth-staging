# Adapted from private unslothai/scripts@494be796ead0d6bfe41a2b2f4d133fe013d4c62f: scripts/studio_test_kit/lifecycle.py
...
def detached_launch_cmd(bin_path: str, port: int, log_path: Path) -> list[str]:
    """The argv that starts Studio detached, for whichever platform is running.

    `setsid` is util-linux and does not exist on macOS, where the Popen died with
    FileNotFoundError before Studio was ever started -- and `start_new_session=True`
    already gives the same new session, so it is only ever belt and braces. Where it
    is missing the log is redirected straight to the file rather than piped through
    `tee`: a long-lived server writing into a pipeline whose reader goes away takes
    BrokenPipeError/500s, which is the failure this whole helper exists to avoid.
    """
    run = f"{shlex.quote(bin_path)} studio -p {port}"
    if shutil.which("setsid"):
        return ["setsid", "-f", "bash", "-c",
                f"{run} 2>&1 | tee -a {shlex.quote(str(log_path))}"]
    return ["bash", "-c", f"{run} >> {shlex.quote(str(log_path))} 2>&1"]
...
def launch_studio(
    ...
    cmd = detached_launch_cmd(bin_path, port, log_path)
    subprocess.Popen(
        cmd,
        env={**os.environ, **env},
