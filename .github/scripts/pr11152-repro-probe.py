...
async def auth_init(base_url: str, password: str, providers: list | None = None) -> str:
    async with httpx.AsyncClient(timeout=30) as client:
        response = await client.post(f"{base_url}/api/auth/login", json={"username": "unsloth", "password": password})
    ...
    auth = StudioAuth(access_token=body["access_token"], refresh_token=body.get("refresh_token", ""), base_url=base_url)
...
CODE = "4817-KESTREL"
TOOL = {"name": "search_kb", "description": "Search the knowledge base.", "input_schema": {"type": "object", "properties": {"q": {"type": "string"}}, "required": ["q"]}}
SEARCH_RESULT = {"type": "search_result", "source": "kb://vault", "title": "Vault access", "content": [{"type": "text", "text": f"The vault access code is {CODE}."}]}
DOCUMENT = {"type": "document", "title": "Vault memo", "source": {"type": "text", "media_type": "text/plain", "data": f"Memo: the vault access code is {CODE}."}}
QUESTION = "What is the vault access code? Reply with the code only."


def tool_turns(block: dict) -> list:
    return [
        {"role": "user", "content": QUESTION},
        {"role": "assistant", "content": [{"type": "tool_use", "id": "toolu_1", "name": "search_kb", "input": {"q": "vault access code"}}]},
        {"role": "user", "content": [{"type": "tool_result", "tool_use_id": "toolu_1", "content": [block]}]},
    ]
...
async def scenario_local_chat(base_url: str, api_key: str, browser_name: str, artifact_dir: Path, password: str | None) -> None:
    ...
    async with httpx.AsyncClient(timeout=300) as client:
        for name, extra in CASES:
            payload = {"model": LOCAL_MODEL, "max_tokens": 48, "temperature": 0, "thinking": {"type": "disabled"}, **extra}
            response = await client.post(f"{base_url}/v1/messages", headers=headers, json=payload)
            try:
                body = response.json()
            except ValueError:
                body = {"raw": response.text[:2000]}
            text = "".join(b.get("text", "") for b in body.get("content", []) if isinstance(b, dict)) if isinstance(body, dict) else ""
            usage = body.get("usage", {}) if isinstance(body, dict) else {}
            results[name] = {"status": response.status_code, "text": text, "usage": usage, "body": body}
            found = CODE.split("-")[0] in text
            print(f"RESULT {name}: status={response.status_code} input_tokens={usage.get('input_tokens')} code_found={found} text={text.strip()[:160]!r}", flush=True)
            if response.status_code != 200:
                failures.append(f"{name}: HTTP {response.status_code} {str(body)[:300]}")
            elif not found:
