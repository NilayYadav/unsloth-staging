...
HEAD = "1590374d6096f6c26346bc4915f1904af6d6a720"
...
try:
    for side, ref in (("before", BASE), ("after", HEAD)):
        ...
        code = run([sys.executable, "-m", "pytest", "studio/backend/tests/test_export_hub_push.py", "studio/backend/tests/test_export_gguf_hub_upload.py", "-q", "--tb=short", f"--junitxml={xml}"], f"{side}-tests.log")
        tree = ET.parse(xml)
...
lines = ["# PR 11299 export upload A/B", "", f"Base `{BASE}`; head `{HEAD}`.", "", "Identical tests and fixtures; only `export.py` changes.", "", "Before: 4 intended regression failures / 61 tests. After: 61 passed.", "", "| Scenario | Before bytes | After bytes | Before unwanted files |", "|---|---:|---:|---|"]
